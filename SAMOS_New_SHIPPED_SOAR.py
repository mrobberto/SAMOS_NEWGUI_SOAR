...
"""
from SAMOS_SOAR_dev.soar_tcs import SoarTCS    # !!! VARIANT CLASS SOAR VS TEST
...
bright_blues = []
for c in gcolors.color_list:
    if (gcolors.color_dict[c][0] <= r_ulim and
        gcolors.color_dict[c][1] >= g_llim and
        gcolors.color_dict[c][2] >= b_llim or
        c == 'blueviolet' or c == 'cadetblue1'): 
        bright_blues.append(c) 
...
print("line 269 end initial sequence")
class App(tk.Tk):
    """ to be written """
...
class ConfigPage(tk.Frame):
 ...
 #       self.PAR = SAM_Par_module.SAMOS_Parameters()
        ...
        #set a key to share with the other classes if the CCD is alive or not    
        #initiated to "no answer"
        self.CCD_answer=''
        self.CCD_is_on = False
...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        r1.place(x=20, y=0)
        r2 = tk.Radiobutton(self.labelframe_Servers, text='Test (No TCS,SAMI)',
...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        LogBookInitialize_Button.place(x=70, y=5)
...
# open file for writing, "w" is writing
    ...
    def load_IP_default(self):
        ...
        if self.PAR.inoutvar.get() == 'inside':
            ...
            self.PAR.IP_dict['IP_CCD'] = dict_from_csv['IP_CCD']
            self.PAR.IP_dict['IP_DMD'] = dict_from_csv['IP_DMD']
            self.PAR.IP_dict['IP_SOAR'] = ""
            self.PAR.IP_dict['IP_SAMI'] = ""
...
# CCD alive?
        ...
        self.CCD_answer = (CCD.get_url_as_string(url_name))[:6]  # expect <HTML>
        print("CCD returns:>", self.CCD_answer, "<")
        if str(self.CCD_answer) == '<HTML>':
            print("CCD is on")
            self.CCD_is_on = True
...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        #self.OFFSET_MOVE_msg.set("RA 0.00 DEC 0.00")
        self.OFFSET_MOVE_RA_msg = tk.StringVar()
        self.OFFSET_MOVE_RA_msg.set("0.00")
        entry_OFFSET_MOVE_RA = tk.Entry(
            self.frame0l, textvariable=self.OFFSET_MOVE_RA_msg, width=6,  bd=3)
        entry_OFFSET_MOVE_RA.place(x=60, y=68)
        
        self.OFFSET_MOVE_DEC_msg = tk.StringVar()
        self.OFFSET_MOVE_DEC_msg.set("0.00")
        entry_OFFSET_MOVE_DEC = tk.Entry(
            self.frame0l, textvariable=self.OFFSET_MOVE_DEC_msg, width=6,  bd=3)
        entry_OFFSET_MOVE_DEC.place(x=125, y=68)
        ...
        self.Focus_variable.set("relative")
        ...
        button_Focus_MOVEREL = tk.Radiobutton(self.frame0l,
                   ...
                   value="relative").place(x=200, y=100)
        button_Focus_MOVEABS = tk.Radiobutton(self.frame0l,
                   ...
                   value="abssolute").place(x=300, y=100)
...
######################################
    ...
    def Offset_option_TCS(self, target):
        """
        ...
        """
        if target == None:
            offset_RA = self.OFFSET_MOVE_RA_msg.get()
            offset_DEC = self.OFFSET_MOVE_DEC_msg.get()   
            target = { "offset_ra": float(offset_RA), "offset_dec": float(offset_DEC) }
        msg_back = SOAR.offset(target)
        self.text_area.insert(tk.END, '\nsent: \n>' +
                              str(target)+'\nreceived: \n>'+str(msg_back))
        self.text_area.yview(tk.END)
    ...
    def Focus_option_TCS(self, event):
        ...
        >calls: focus( value, move_type="absolute")
        Parameters
        ----------
        event : TYPE
            DESCRIPTION.
    ...
    def Whitespot_option_TCS(self, message):
        ...
        calls> whitespot(self, percentage, turn_on=True):
        where    
            percentage : int
        ...
        if message != "ON":
            ...
            turn_on = True
        msg_back = SOAR.whitespot(percentage, turn_on)
        self.text_area.insert(tk.END, '\nsent: \n>' +
    ...
    def Lamp_LN_option_TCS(self, status):
        ...
        calls> lamp(self, lamp, state="ON", percentage=0):
        where
            lamp : str or int
        ...
        """
        lamp = self.Lamp_number.get()
        percentage = ''
        if ((status == "ON") and ((lamp == 'L9') or (lamp == 'L12'))):
            percentage = self.Lamp_percentage.get()
        location = lamp + ' ' + percentage
        message = "LAMP " + status + ' ' + location
        
        msg_back = SOAR.lamp(lamp, status,percentage)
    ...
    def ADC_option_TCS(self, message):
        """ This command enable or disable the Whitespot device. 
        #calls> adc(self, percentage, park=False):
        where   
        ...
        if message != "IN":
            ...
            park = False    
        msg_back = SOAR.adc(float(percentage),park)
        self.text_area.insert(tk.END, '\nsent: \n>' +
                              "ADC " + percentage + " " + park + '\nreceived: \n>'+str(msg_back))
        self.text_area.yview(tk.END)
    ...
    def IPA_option_TCS(self):
        """
        This command set a new Instrument Position Angle (IPA) to the TCS.
        The IPA is given in units of degrees.
        #calls> ipa(self, angle):
        where
            ----------
            angle : float
                The IPA angle to set in degrees.
        """
        angle = self.IPA_MOVE_msg.get()
        msg_back = SOAR.ipa(angle)
        message = "IPA " + angle
        self.text_area.insert(tk.END, '\nsent: \n>' +
                              message+'\nreceived: \n>'+str(msg_back))
        self.text_area.yview(tk.END)
    ...
    def Instrument_option_TCS(self, instrument):
        """
        This command selects the instrument in use
        #calls> instrument(self, instrument):
        wwhere   
            instrument : str
                The name of the instrument to set.
        """
        msg_back = SOAR.instrument(instrument)
    ...
    def Target_option_TCS(self, event):
        ...
        target = { "ra": float(RA), "dec": float(DEC), "epoch": float(EPOCH)}
        #if event != "MOVE":
        #    RADEC = ''
        msg_back = SOAR.target(target)
...
class MainPage(tk.Frame):
    ...
    def __init__(self, parent, container):
        ...
        self.SOAR_PAGE = SOAR_Page(parent,container)
...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        check_Logbook_Yes.place(x=285, y=-5)
        ...
        button_ExpStart.place(x=285, y=20)
        ...
        self.exp_progbar.place(x=280, y=60)
        ...
        self.readout_progbar.place(x=280, y=90)
        # need to save the style so it can be configured later during exposure/readout
        ...
        """"Creating a Checkbutton for the flipped image on SAMI port"""
        self.SAMOS_flipped =  True                
        self.checkbutton_SAMOS_flip_2 = tk.Button(labelframe_ExposeBegin,
                                             text = "Image Flip is On",
                                             fg = "green",
                                             relief = "raised",
                                             bd=0,
                                             command=self.set_SAMOS_flip_2)
        self.checkbutton_SAMOS_flip_2.place(x=300, y=115)
...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        self.frame_FITSmanager.place(
            x=10, y=600, anchor="nw", width=430, height=238)
...
#        self.griz_menu_Survey.place(x=190, y=65)
        ...
        """ GSProbe center box"""
        label_GS00 = tk.Label(
            labelframe_Query_Survey, text="GS(0,0):")
        label_GS00.place(x=225, y=36)
        self.GS00x = tk.IntVar()
        entry_GS00x = tk.Entry(
            labelframe_Query_Survey, width=3,  bd=3, textvariable=self.GS00x)
        entry_GS00x.place(x=275, y=35)
        self.GS00x.set(480)  #DEFAULT
        self.GS00y = tk.IntVar()
        entry_GS00y = tk.Entry(
            labelframe_Query_Survey, width=3,  bd=3, textvariable=self.GS00y)
        entry_GS00y.place(x=325, y=35)
        self.GS00y.set(490) #DEFAULT
        
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
#
#       RA, DEC center display
#
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        self.string_RA_cntr_arcs = tk.StringVar()
        self.string_RA_cntr_arcs.set(0)
        label_RA_cntr_arcs = tk.Label(labelframe_Query_Survey, text='X(arcs):',  bd=3)
        self.entry_RA_cntr_arcs = tk.Entry(
            labelframe_Query_Survey, width=5,  bd=3, textvariable=self.string_RA_cntr_arcs)
        label_RA_cntr_arcs.place(x=150, y=65)
        self.entry_RA_cntr_arcs.place(x=200, y=65)
        ...
        self.string_DEC_cntr_arcs = tk.StringVar()
        self.string_DEC_cntr_arcs.set(0)
        label_DEC_cntr_arcs = tk.Label(labelframe_Query_Survey, text='Y(arcs):',  bd=3)
        self.entry_DEC_cntr_arcs = tk.Entry(
            labelframe_Query_Survey, width=5,  bd=3, textvariable=self.string_DEC_cntr_arcs)
        label_DEC_cntr_arcs.place(x=150, y=90)
        self.entry_DEC_cntr_arcs.place(x=200, y=90)
        
        button_RADEC_to_SOAR = tk.Button(labelframe_Query_Survey, text="Center Telescope", bd=3,
                             command=self.send_RADEC_to_SOAR)
...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        self.canvas_Indicator.place(x=60, y=835, width=310, height=65)

        self.canvas_Indicator.create_oval(20, 10, 50, 40, fill=indicator_light_on_color,
                                          outline=None, tags=["filter_ind"])
        self.canvas_Indicator.create_text(40, 50, text="Filters")
        self.canvas_Indicator.create_oval(100, 10, 130, 40, fill=indicator_light_on_color,
                                          tags=["grism_ind"], outline=None)
        self.canvas_Indicator.create_text(120, 50, text="Grisms")

        # indicator for mirror and SOAR TCS applicable at telescope
        self.canvas_Indicator.create_oval(170, 10, 200,40, fill=indicator_light_off_color, tags=["mirror_ind"],
                                          outline=None)
        self.canvas_Indicator.create_text(190, 50, text="Mirror")
        self.canvas_Indicator.create_oval(240, 10, 270, 40, fill=indicator_light_off_color, tags=["tcs_ind"],
                                          outline=None)
        self.canvas_Indicator.create_text(260, 50, text="TCS")
...
#
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        self.Button_PSF = tk.Button(
            self, text="get PSF", command=self.multi_PSF, relief=tk.RAISED, fg="blue")
        self.Button_PSF.place(x=460, y=550)
...
#        return
    ...
    def write_GingaRegions_ds9adFile(self):
        ...
        if "RRR_RADec" not in dir(self):
            ...
            """ it seems a good idea to save adding the name of the imaging file"""
            if self.filename_regfile_RADEC != '':
                path, filename= os.path.split(self.filename_regfile_RADEC)
                s = re.search(r'RADEC=',filename)
                TOP = filename[:s.end()-6]
                pretty_file_number = "{:03d}".format(self.out_fnumber.get())
                file = os.path.join(path,
                                    TOP+str(pretty_file_number)+"_"+filename[s.end()-6:])                
            else:
...
#        self.textbox_filename_slits.delete('1.0', tk.END)
        self.filename_regfile_RADEC = filedialog.askopenfilename(initialdir=os.path.join(parent_dir, "..", "Science_targets", "RADEC"),
                                                                 title="Select a File",
                                                                 filetypes=(("Text files",
                                                                             "*.reg"),
                                                                            ("all files",
...
#        DMD.apply_shape(self.slit_shape)
    ...
    def push_slits(self):
        ...
        #ADDING HERE THE SWITCH FOR FLIPPED IMAGES 
        print("line 6709: self.ConfP.SAMOS_flipped", self.SAMOS_flipped)
        DMD.apply_shape(self.slit_shape,self.SAMOS_flipped)
        # DMD.apply_invert()
...
#        self.Echo_String.set(t)
    ...
    def save_canvas(self):
        ...
        regs = ap_region.ginga_canvas_object_to_astropy_region(self.canvas, logger=self.logger)
...
#            imtype = "dark_{}s".format(self.ExpTime.get())
        ...
        """CHECK IF THERE IS CCD CONNECTION
       	 IF THERE IS NO CONNECTION, WE ARE JUST TESTING;
     	PRESSING  >START< LOADS A TEST FILE FROM THE DISK"""
        """
        if self.ConfP.CCD_is_on==False: 
        #if CCD is not connected…
        #if  self.PAR.IP_status_dict['IP_CCD'] == False:
            #and IF SAMOS is mounted on SAMI port, the images arrive flipped: 
            # => load a simulated flipped one!
            if self.ConfP.SAMOS_flip.get():
                newfiles = [filedialog.askopenfilename(filetypes=[("allfiles", "*"),
                                                      ("fitsfiles", "*.fits")])]
                #self.open_file()     
                #newfiles = self.AstroImage
        else:        
        # this is the original part, runs if the CCD is connected.
...
#         FOR TEST  
#        newfiles = ["/Users/robberto/Desktop/SAMOS_DATA/SAMOS_20241014/sci_SLOAN-r_test_0070.fits"]
...
#        Camera = Class_Camera(dict_params=params)
        for fname in newfiles:
            ...
            # start here the variant for the flipped image
            # from PIL import Image  #check if already imported

            if self.SAMOS_flipped == True: 
            #if CCD is not connected…
...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
    ...
    def set_SAMOS_flip_2(self):
        """flip the CCD flip"""
        if self.SAMOS_flipped:
            self.checkbutton_SAMOS_flip_2.config(
                                                 text = "Image Flip  is Off", 
                                                 fg = "red")
            self.SAMOS_flipped = False
        else:     
            self.checkbutton_SAMOS_flip_2.config(
                                                 text = "Image Flip is On", 
                                                 fg = "green")
            self.SAMOS_flipped= True
        print("self.SAMOS_flipped is ", self.SAMOS_flipped)    
...
#        main_fits_header.set_param("filterpos", self.selected_FW_pos.get())
        ...
        dmd_hdu = None
        if hasattr(DMD, 'current_dmd_shape'):
#        if DMD.current_dmd_shape is not None:
            dmd_hdu = self.create_dmd_pattern_hdu(
                main_fits_header.output_header)
...
#        self.entry_out_fnumber.invoke("buttonup")
        """ before exiting we set up the  self.fits_image_ql file, 
        used e.g. by the twirl procedure"""
        self.fits_image_ql = QL_fname             
...
#        image = load_data(fits_image_converted, logger=self.logger)
    ...
    def load_existing_file(self):
        ...
        self.last_fits_file_dialog = filedialog.askopenfilename(initialdir=os.path.join(parent_dir, "..", "Science_targets"),
        ...
        if self.SAMOS_flipped == 1:
            self.fitsimage.transform(True,False,False)
...
#        self.root.title(self.fullpath_FITSfilename)
    ...
    def send_RADEC_to_SOAR(self):
        ...
        d_ra = self.string_RA_cntr_arcs.get()
        d_dec = self.string_DEC_cntr_arcs.get()
...
#            data = hdu_in[0].data
    ...
    def twirl_Astrometry(self):
        ...
        if  self.string_RA.get() !=  '' and self.string_DEC.get() != '':        
            ra = self.string_RA.get()
            dec = self.string_DEC.get()
            print("RA and DEC read from the text box")
            
        elif self.PAR.inoutvar.get() == "inside": 
            infoa_dict = self.SOAR_PAGE.Handle_Infox('INFOA')
            ra=infoa_dict['MOUNT_RA']
...
#        this_region = CirclePixelRegion(center_pix, radius_pix)
        ...
        try:
            ...
            print("internet not available, reading GAIA sources from local disk")
            Science_targets_dir = os.path.join("..", "Science-tagets")
            gaias_infield_file = filedialog.askopenfilename(initialdir=Science_targets_dir,                                                          title="Select a File",
                                                             filetypes=(("csv files",
                                                                         "*.csv"),
                                                                        ("all files",
                                                                         "*.*")))
            gaias_list = list()
            with open(gaias_infield_file, newline='') as csvfile:
                spamreader = csv.reader(csvfile, delimiter=' ', quotechar='|')
                next(spamreader, None)  # skip the headers
                for row in spamreader:
                    ra=np.float_((row[0].split(","))[2])
                    dec=np.float_((row[0].split(","))[3])
                    gmag = np.float_((row[0].split(","))[4])
                    if gmag < 18:
                        print(ra,dec)
                        gaias_list.append([ra,dec])
                gaias = np.array(gaias_list[0:self.nrofstars.get()])      
...
#        gaias_sorted = np.sort(gaias,axis=0)
        ...
        print("working on GAIA")
        self.wcs = twirl.compute_wcs(stars, 
                                     gaias[0:self.nrofstars.get()])#,tolerance=0.1)
...
#        plt.figure(figsize=(8,8))
        ...
        """ Calculate the position angle 
             from the original paper of Bob Hanish
             https://lweb.cfa.harvard.edu/~jzhao/SMA-FITS-CASA/docs/wcs88.pdf
        """
        
        CD = self.wcs.wcs.cd
        CDELT1 = np.sqrt(CD[0,0]**2+CD[1,0]**2)
        cos_theta = CD[0,0]/CDELT1
        PA = np.arccos(cos_theta)*180/np.pi
        print(f'\nPosition Angle: {PA:6.2f} degrees \n'  )
        if self.wcs is None:
        ...
        """ get Guide Star Probe Offset"""
        GS_00x = self.GS00x.get()
        GS_00y = self.GS00y.get()
        print("pixel of the guide probe (0,0) point:",GS_00x,GS_00y)
        ra_GS00x, dec_GS00y = self.wcs.all_pix2world([[GS_00x,GS_00y]], 0)[0]
        print("RADEC of the guide probe (0,0) point:", ra_GS00x,dec_GS00y)
        print("check inverse transformation:")
        ad = np.array([[ra_GS00x,dec_GS00y]]).astype(float)
        print("xy:",self.wcs.all_world2pix(ad,0)[0])

        #mywcs = wcs.WCS(header)
        #ra, dec = self.wcs.all_pix2world([[data.shape[0]/2,data.shape[1]/2]], 0)[0]
        print("RADEC of the current pointing:", ra,dec)
        ad = np.array([[ra,dec]]).astype(float)
        x_tel,y_tel = self.wcs.all_world2pix(ad,0)[0]
        print("pixel of the current telescope pointing:",x_tel,y_tel)
        #ra_target = self.string_RA.get()
        #dec_target = self.string_DEC.get()
        #print("RADEC of the target commanded:", ra_target,dec_target)


        """display RADEC of current pointing  """
        self.string_RA_cntr.set(ra)
        self.string_DEC_cntr.set(dec)
        """determine the offfset between current and target (GS) field center"""
        Delta_RA = float(ra_GS00x) -  float(ra)
        Delta_DEC =  float(dec_GS00y) - float(dec)
        print("difference between current pointing and GP(0,0):", Delta_RA,Delta_DEC)
        print("difference between current pointing and GP(0,0) in Equatorial arcsec :", Delta_RA*3600.,Delta_DEC*3600.)
        Delta_RA_arcsec_TP = Delta_RA*3600.*np.cos(dec*math.pi/180.)
        Delta_DEC_arcsec_TP = Delta_DEC*3600.
        print("difference between current pointing and GP(0,0) in Equatorial arcsec :", Delta_RA_arcsec_TP,Delta_DEC_arcsec_TP)
        ...
        self.string_RA_cntr_arcs.set(Delta_RA_arcsec_TP)
        self.string_DEC_cntr_arcs.set(Delta_DEC_arcsec_TP)
...
#        regions = [CirclePixelRegion(center=PixCoord(x, y), radius=radius_pix)
    ...
    def multi_PSF(self):
        ...
        print("values calculated using ",len(fwhm_x),"stars")
...
#        dmd_text = "DMD_X: %i  DMD_Y: %i \n"%(np.round(dmd_x), round(dmd_y))
        ...
        """flip pixels and mirrors if the image has been flipped""" 
        if self.SAMOS_flipped:
            """
            dmd_text = "DMD_X: %i  DMD_Y: %i \n" % (
                1080-np.floor(dmd_x), np.floor(dmd_y))
            text = "X: %i  Y: %i  Value: %s" % (
                1032-np.floor(fits_x), np.floor(fits_y), value)
        else:
            """
            dmd_text = "DMD_X: %i  DMD_Y: %i \n" % (
...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        self.frame_FITSmanager = tk.Frame(self, background="pink")
        self.frame_FITSmanager.place(
            x=10, y=10, anchor="nw", width=420, height=200)
...
#        image = load_data(fits_image_converted, logger=self.logger)
    ...
    def load_existing_file(self):
        """ to be written """
        #FITSfiledir = os.path.join(self.PAR.QL_images)
        self.last_fits_file_dialog = filedialog.askopenfilename(initialdir=self.fits_dir,
                                                                 title="Select a File",
                                                                 filetypes=(("fits files",
                                                                             "*.fits"),
                                                                            ("all files",
