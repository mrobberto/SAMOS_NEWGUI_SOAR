...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        self.SAMOS_flip = tk.IntVar()
        checkbutton_SAMOS_flip = tk.Checkbutton(self.labelframe_Servers, 
                                                command = self.set_SAMOS_flip,
                                                text="SAMOS_flip", 
                                                variable=self.SAMOS_flip, 
                                                onvalue=1, 
                                                offvalue=0)
        checkbutton_SAMOS_flip.place(x=300, y=0)
        checkbutton_SAMOS_flip.select()
...
#            SF.read_IP_initial_status()
    ...
    def set_SAMOS_flip(self):
        """flip the CCD flip"""
        if self.SAMOS_flip.get() == 1:
            self.SAMOS_flip.set = 0
        if self.SAMOS_flip.get() == 0:
            self.SAMOS_flip.set = 1
        print("flip status out:", self.SAMOS_flip.get())
...
######################################
    ...
    def Offset_option_TCS(self, target):
        ...
        """
        #target = { "offset_ra": float, "offset_dec": float }
...
#        self.Offset_option_msg.set("you selected "+self.Offset_option_selected.get())
    ...
    def Focus_option_TCS(self, event):
        ...
        msg_back = SOAR.focus(offset, move_type)
        self.text_area.insert(tk.END, '\nsent: \n>' +
                              message+'\nreceived: \n>'+str(msg_back))
        self.text_area.yview(tk.END)
    ...
    def ADC_option_TCS(self, message):
        ...
        if message != "IN":
            ...
            park = False    
        msg_back = SOAR.adc(percentage,park)
        self.text_area.insert(tk.END, '\nsent: \n>' +
                              "ADC " + percentage + " " + park + '\nreceived: \n>'+str(msg_back))
        self.text_area.yview(tk.END)
...
#        self.perform_DMDcalibration = tk.IntVar()
#        check_Buffer = tk.Checkbutton(
#            labelframe_ExposeBegin, text='DMD calib', variable=self.perform_DMDcalibration, onvalue=1, offvalue=0)
#        check_Buffer.place(x=240, y=115)
